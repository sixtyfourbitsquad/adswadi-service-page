...
function buildPayHref(label: string, option: GoogleAgencyOption) {
  const name = `Google Agency – Monthly – ${label}`;
  const params = new URLSearchParams();
  params.set("service", "google-agency-monthly");
  params.set("name", name);
  if (option.amount && option.amount.trim()) params.set("amount", option.amount.trim());
  return `/payment?${params.toString()}`;
}

export default function GoogleAgencyMonthlyPage() {
  ...
  const monthly = config.googleAgencyMonthly ?? DEFAULT_OPTIONS;
  const indian = monthly.indian ?? DEFAULT_OPTIONS.indian;
  const international = monthly.international ?? DEFAULT_OPTIONS.international;
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#F5F3FF] via-[#EDE9FE] to-[#DBEAFE]">
      <div className="max-w-4xl mx-auto px-4 pt-10 pb-20">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-gradient-brand text-center mb-10">
          Google Agency Ads Account – Monthly
        </h1>

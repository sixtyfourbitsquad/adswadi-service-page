...
function buildPayHref(label: string, option: GoogleAdsOption) {
  const name = `Google Ads – Weekly – ${label}`;
  const params = new URLSearchParams();
  params.set("service", "google-ads-weekly");
  params.set("name", name);
  if (option?.amount?.trim()) params.set("amount", option.amount.trim());
  return `/payment?${params.toString()}`;
}

export default function GoogleAdsWeeklyPage() {
  ...
  const indian = detail?.weekly?.indian ?? { price: "", amount: "" };
  const international = detail?.weekly?.international ?? { price: "", amount: "" };
